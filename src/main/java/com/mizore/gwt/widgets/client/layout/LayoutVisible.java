 ...
 * and pass it to {@link #LayoutVisiblePanel(Element)}. Rather than attaching child elements
 * directly to the element managed by this {@link LayoutVisible}, use the
 * {@link LayoutVisible#attachChild(Element)} method. This will attach the child
 * element and return a {@link LayoutVisible.Layer} object which is used to manage the
 * child.
 * </p>
 * 
 * <p>
 * A separate {@link LayoutVisible.Layer} instance is associated with each child
...
public class LayoutVisible {
	...
	/**
	 * Callback interface used by {@link LayoutVisible#layout(int, AnimationCallback)}
	 * to provide updates on animation progress.
	 */
	public interface AnimationCallback {

		/**
		 * Called immediately after the animation is complete, and the entire
		 * layout is in its final state.
		 */
	 ...
	 * <ul>
	 * <li>{@link LayoutVisible.Layer#setLeftRight}</li>
	 * <li>{@link LayoutVisible.Layer#setLeftWidth}</li>
	 * <li>{@link LayoutVisible.Layer#setRightWidth}</li>
	 * <li>{@link LayoutVisible.Layer#setTopBottom}</li>
	 * <li>{@link LayoutVisible.Layer#setTopHeight}</li>
	 * <li>{@link LayoutVisible.Layer#setBottomHeight}</li>
	 * </ul>
	 * </p>
	 * 
	 * <p>
	 * By default, each layer is set to fill the entire parent (i.e., {left,
	 * top, right, bottom} = {0, 0, 0, 0}).
	 * </p>
	 */
	public class Layer {
		...
		boolean setTargetLeft = true, setTargetRight = true,
				setTargetTop = true, setTargetBottom = true, setTargetWidth,
				setTargetHeight;
		 ...
		 * 
		 * <p>
		 * This is the element that sits between the parent and child elements.
		 * It is normally necessary to operate on this element only when you
		 * need to modify CSS properties that are not directly modeled by the
		 * Layer class.
		 * </p>
		 * 
		 ...
		 * @param heightUnit
		 */
		public void setBottomHeight(double bottom, Unit bottomUnit,
	...
	private LayoutVisibleImpl impl = GWT.create(LayoutVisibleImpl.class);
	 ...
	 * 
	 * @param parent
	 *            the element to serve as the layout parent
	 */
	public LayoutVisible(Element parent) {
	 ...
	 * current parent element. Use the {@link Layer} it returns to manipulate
	 * the child.
	 * </p>
	 * 
	 * @param child
	 ...
	 *            the child to be attached
	 * @param before
	 *            the child element before which to insert
	 * @return the {@link Layer} associated with the element
	 */
	 ...
	 *            the child to be attached
	 * @param userObject
	 *            an arbitrary object to be associated with this layer
	 * @return the {@link Layer} associated with the element
	 */
	 ...
	 * such as {@link Unit#EM}, and {@link Unit#EX}, will return different
	 * values based upon the parent's associated font size. {@link Unit#PCT} is
	 * dependent upon the parent's actual size, and the axis to be measured.
	 * </p>
	 * 
	 * @param unit
	 *            the unit type to be measured
	 * @param vertical
	 ...
	 * frame of the animation, and upon completion.
	 * 
	 * @param duration
	 *            the duration of the animation
	 * @param callback
	...
	public void layout(int duration, final AnimationCallback callback) {
		...
		// If there's no actual animation going on, don't do any of the
		// expensive
		...
		animation = new Animation() {
			...
			protected void onUpdate(double progress) {
				for (Layer l : layers) {
					...
					if (l.setTargetRight) {
						l.right = l.sourceRight
								+ (l.targetRight - l.sourceRight) * progress;
					}
					if (l.setTargetTop) {
						l.top = l.sourceTop + (l.targetTop - l.sourceTop)
								* progress;
					}
					if (l.setTargetBottom) {
						l.bottom = l.sourceBottom
								+ (l.targetBottom - l.sourceBottom) * progress;
					}
					if (l.setTargetWidth) {
						l.width = l.sourceWidth
								+ (l.targetWidth - l.sourceWidth) * progress;
					}
					if (l.setTargetHeight) {
						l.height = l.sourceHeight
								+ (l.targetHeight - l.sourceHeight) * progress;
					}
	...
	/**
	 * This method must be called when the parent element becomes attached to
	 * the document.
	 * 
	 * @see #onDetach()
	 */
